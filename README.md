...
## FastDVDnet
FastDVDnet is a deep-learning-based video denoising algorithm designed to remove noise from video sequences efficiently and effectively.  
Although more recent methods outperform FastDVDnet by 1-2 PSNR at most, they actually introduce huge models and heavy operations so they are not as efficient as FastDVDnet in terms of Real-Time applications.  
FastDVDnet consists of a cascaded two-step architecture based on U-Net blocks. The input contains 5 frames for time-step in order to apply spatio-temporal denoising.  
The model is originally designed and trained to predict and remove gaussian noise from video, so we have simply modified the architecture to be able to process different noise types as in the real world.
<img src="https://github.com/m-tassano/fastdvdnet/raw/master/img/arch.png" height=350>

## Noise Generator
Thanks to the idea of the [Noise Generator](https://github.com/SFU-Multimedia-Lab/noise_generator) provided by SFU_Multimedia_Lab, we have being able to simulate real-world noise in images and videos. We improve their generator implementing specific functions to apply the noise generation directly on GPU during the training process, in order to obtain noise generated by different parameters at each batch processing.
